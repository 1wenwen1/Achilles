...
parser.add_argument("--mkp",        action="store_true")     # run AWS
parser.add_argument("--mkpp",       action="store_true")     # run AWS
parser.add_argument("--p",          type=int, default=0)     # mode
parser.add_argument("--numtrans",   type=int, default=0)     # batchsize
...
if args.p>=0:
    p = args.p
    print("SUCCESSFULLY PARSED ARGUMENT - the p will be:", p)
...
if args.test:
    ...
    print("mkparams Ali experiment")
    if p ==0:
        mkApp(Protocol.CHCOMB,2,faults[0],numTrans,payloadSize,pct)
    if p ==1:
        mkApp(Protocol.CHCOMB,3,faults[0],numTrans,payloadSize,pct)
    if p ==4:
        mkApp(Protocol.CHCOMB,2,faults[0],numTrans,payloadSize,pct)
    if p ==5:
        mkApp(Protocol.CHCOMBDBG,2,faults[0],numTrans,payloadSize,pct)
